...
export default {
  ...
  index: {
    metaDescription:
      '面向严苛工业现场的安全本地部署（on-prem）边缘计算：IP67 铝合金外壳中的可编程 M12-PoE 交换机、路由器与计算模块。',
    heroTitle: 'Edge-Compute',
    taglinePre: '面向严苛工厂环境的安全本地部署边缘节点：面向工业边缘应用的可编程 M12-PoE 交换机、路由器与计算模块——由德国 ',
    ...
    features: [
      {
        icon: '🌊',
        title: '专为严苛工业现场打造',
        text: '阳极氧化铝外壳，M12 连接器全密封，IP67 防护等级。抗冲击、防尘、防潮，可在最高 1 bar 的水下环境中运行。',
      },
      {
        icon: '🔌',
      ...
      cards: [
        ...
        },
        {
          ...
          paras: [
            ...
            '得益于现代化的安全架构，一切均在本地（on-premises）运行：除非您主动允许，否则任何原始数据都不会进入您的上游网络或任何云端。',
          ],
        },
      ],
    ...
    gallery: {
      title: '为严苛工厂环境而生',
      subtitle: '一整块阳极氧化铝。每个端口皆密封。随时可装入您的机柜、机器或水箱——即便是在最严苛的工业环境中。',
      figures: [
        {
    ...
    closer: {
      title: '让算力贴近您的测量。',
      text: '无论是连接单个传感器，还是为整座工厂部署仪表：skAInet Edge-Compute 都为您提供一个坚固、安全的本地部署边缘平台——开放，且经现场验证。告诉我们您想测量什么——我们会告诉您如何实现。',
    },
  },
    ...
    items: [
      {
        ...
        paras: [
          ...
          '每一次转辙动作都会产生特征性的电流与功率信号。通过在轨旁持续采集这些数据，而不是仅依赖定期巡检，可将每组道岔纳入持续状态监测体系，使磨损和性能劣化能够在故障发生之前及早识别。',
        ],
        bullets: [
      ...
      },
      {
        ...
        paras: [
          ...
          '借助联邦学习，模型在数据产生的现场直接训练。Edge-Compute 的两个相互隔离的以太网接口天然适配这一架构：传感器数据留在 LAN 侧，只有模型更新——从不传输原始测量数据——才会进入上游网络。',
        ],
        bullets: [
          '边缘端声学与振动数据分析',
          '联邦学习训练——原始数据不离开现场',
      ...
      },
      {
        id: 'panda',
        sector: '基础科研 · 加速器物理',
        title: '控制 GSI/FAIR 的 PANDA 亮度监测器（luminosity detector）',
        paras: [
          'Edge-Compute 与波鸿鲁尔大学 EP1 教研室强子与核物理研究组（Prof. Fritsch）合作，为 GSI/FAIR 及在建的 HESR 储存环监测并控制 PANDA 亮度监测器的线性位移机构。',
          '在加速器环境中，可靠性与远程可操作性至关重要。密封铝外壳、M12 连接器以及支持 EPICS 的开放 Linux 系统，使 Edge-Compute 非常适合用于探测器控制。',
        ],
        bullets: [
          '监测与控制精密机械定位系统',
          '集成到实验控制系统（EPICS）中',
